...
public class PrestamoServlet extends HttpServlet {
    ...
    private void agregarPrestamo(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("AgregarPrestamoServlet - parámetros recibidos:");
        ...
        try {
            ...
            System.out.println("AgregarPrestamoServlet - idMaterial parse error: " + idMaterialStr);
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            response.getWriter().write("{\"error\": \"idMaterial inválido\"}");
            return;
        }

        // Crear DtPrestamo usando setters (más seguro que depender de constructor)
        DtPrestamo prestamo = new DtPrestamo();
        try {
            prestamo.setFechaSoli(fechaSoli);
            prestamo.setFechaDev(fechaDev);
            prestamo.setEstadoPres(estado);
            prestamo.setLector(correoLector);
            prestamo.setBibliotecario(correoBiblio);
            prestamo.setMaterial(idMaterial); // <- asegurarse de setear el material explicitamente
        } catch (Exception ex) {
            System.out.println("AgregarPrestamoServlet - error setting DtPrestamo fields: " + ex.getMessage());
        }

        // Log JSON del DTO antes de enviarlo
        try {
            String prestamoJson = objectMapper.writeValueAsString(prestamo);
            System.out.println("AgregarPrestamoServlet - DtPrestamo JSON a enviar: " + prestamoJson);
        } catch (Exception ex) {
            System.out.println("AgregarPrestamoServlet - error serializando DtPrestamo: " + ex.getMessage());
        }

        // Llamar al cliente que invoca el servicio
        prestamoClient.agregarPrestamo(prestamo);
